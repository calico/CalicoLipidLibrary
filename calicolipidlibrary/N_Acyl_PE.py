...
class N_Acyl_PE(NAcylGPL):
		...
		def theoreticalDigest(self):
			...
			if adduct in POS_ADDUCTS:
				...
				if adduct == "[M+H]+":
					FRAGMENTS.append( [PREC-MW("C2H8NO4P"), 1000, "NL ethanolamine-P"] ) #seems wrong
					FRAGMENTS.append( [MW("C2H7NO")+ADDUCT[adduct], 1000, "ethanolamine"] ) #seems wrong
					...
					for i in range(0,len(self.chains)):
						...
						FRAGMENTS.append( [NL(self.chains[i])+PROTON+H2O, 1000, "sn" + chain + " acylium ion+ water"])
						FRAGMENTS.append( [NL(self.chains[i])+PROTON, 1000, "sn" + chain + " acylium ion"])
				...
				if adduct == "[M+Na]+" or adduct == "[M+NH4]+" or adduct == "[M+K]+":  
					...
					for i in range(0,len(self.chains)):
						...
						FRAGMENTS.append( [NL(self.chains[i])+PROTON, 1000, "sn" + chain + " acylium ion"])
						FRAGMENTS.append( [PREC - NL(self.chains[i]), 1000, "NL (sn" + chain ])
						FRAGMENTS.append( [PREC-MW("C2H5N")-NL(self.chains[i]), 1000, "prec-sn" + chain + "-ethanolamine-2PROTON"] ) # FIX
				...
				FRAGMENTS.append( [MW("C5H12NO5P")-PROTON, 1000, "C5H12NO5P"] )  #should probably remove

				if adduct == "[M+FA]-" or adduct == "[M+Cl]-":
					FRAGMENTS.append( [MW("C3H9O6P")-H2O+ADDUCT[adduct], 1000,  "glycerol3P-H2O"] ) 

				FRAGMENTS.append( [MW("C2H8NO4P")-H2O-PROTON,  1000,  "Phosphoethanolamine - H20"] ) #should probably remove
				FRAGMENTS.append( [MW("C2H8NO4P")-PROTON,  1000,  "Phosphoethanolamine"] )  #should probably remove

				FRAGMENTS.append( [NL(self.chains[2])-MW("O")+MW("C2H6O4PN")-PROTON,1000,"ethanolamide+amide"])
				FRAGMENTS.append( [NL(self.chains[2])-MW("O")+MW("C2H6O4PN")+MW("C3H4O")-PROTON,1000,"ethanolamide+amide+glycerol"])				
				for i in [0,1]:
					chain = str(i+1)
					FRAGMENTS.append( [NL(self.chains[i])-PROTON, 1000, "sn" + chain + "-RCO"])
					FRAGMENTS.append( [PREC - NL(self.chains[i])+H2O, 1000, "NL sn" + chain + ""])
					FRAGMENTS.append( [PREC - NL(self.chains[i]), 1000, "NL sn" + chain + "-H2O"])
