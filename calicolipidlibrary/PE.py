    ...
    def theoreticalDigest(self):
        ...
        if adduct in POS_ADDUCTS:
            if adduct == "[M+H]+":
                ...
                FRAGMENTS.append(
                    [NL(self.chains[i]) + PROTON, 13, "sn" + chain + " acylium ion"]
                )
                ...
                # below is probably not a real peak, but added for DIMS ease
                FRAGMENTS.append(
                    [NL(self.chains[i]) + PROTON, 5, "sn" + chain + " acylium ion"]
                )

            if adduct in ["[M+Na]+", "[M+K]+", "[M+Li]+", "[M+NH4]+"]:
                FRAGMENTS.append([PREC, 374, "precursor"])
                FRAGMENTS.append(
                    [MW("PO4H3") + ADDUCT[adduct], 900, "phosphoric acid + adduct"]
                )
                FRAGMENTS.append(
                ...
                FRAGMENTS.append(
                    [PREC - MW("C2H8NO4P") + H2O, 27, "NL deoxyethanolamine-P"]
                )
                FRAGMENTS.append(
                    [
            ...
            FRAGMENTS.append(
                [MW("C5H12NO5P") - PROTON, 27, "ethanaolamine-P + deoxyglycerol"]
            )
            ...
            chain = str(i + 1)
            FRAGMENTS.append(
                [PREC - NL(self.chains[i]) + H2O, 8, "NL sn" + chain + "ketene"]
            )
            ...
            if self.chains[i][1] > 3:
                ...
                FRAGMENTS.append(
                    [
                        NL(self.chains[i]) - MW("CO2") - PROTON,
                        93,
                        "sn" + chain + "decarboxylation",
                    ]
                )
            else:
            ...
            chain = str(i + 1)
            FRAGMENTS.append(
                [PREC - NL(self.chains[i]) + H2O, 45, "NL sn" + chain + "ketene"]
            )
