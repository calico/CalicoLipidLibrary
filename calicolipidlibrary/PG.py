        ...
        if adduct in POS_ADDUCTS:
            if adduct == "[M+H]+":
                ...
                for i in range(0, len(self.chains)):
                    ...
                    FRAGMENTS.append(
                        [NL(self.chains[i]) - H2O + PROTON, 1000, "RCO+ sn" + chain]
                    )
                    FRAGMENTS.append(
                        [
                            PREC - NL(self.chains[i]) + H2O + H2O,
                            1000,
                            "NL sn" + chain + "-H2O",
                        ]
                    )
            ...
            if adduct in ["[M+Na]+", "[M+K]+", "[M+Li]+", "[M+NH4]+"]:
                FRAGMENTS.append(
                    [MW("C3H9O6P") + ADDUCT[adduct], 1000, "glycerol-P + adduct"]
                )
                FRAGMENTS.append(
                    [
                ...
                FRAGMENTS.append(
                    [
                        PREC - MW("C3H9O6P") - ADDUCT[adduct] + PROTON,
                        79,
                        "NL glycerol-P + Adduct",
                    ]
                )
                ...
                FRAGMENTS.append(
                    [PREC - NL(self.chains[i]) + H2O, 1, "NL sn" + chain + " ketene"]
                )
