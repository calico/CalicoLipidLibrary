...
def MW_list(atoms=list()):  #calc MW from list that is C,H,N,O,P,S
        ...
        return 12.000*nC + 1.0078250321*nH + 14.0030740052*nN + 15.9949146221*nO + 30.97376151*nP + 31.972070*nS
...
class Lipid:
	...
	def MF(self):
		...
		if (bbType != "bbIsoprenoid"):
			...
			if len(chainType) == 2 and bbType == "bbGPL" and (self.chains[0][0] == 0 or self.chains[1][0] == 0):   #for Lyso PLs
					formula[O] -= 1
					formula[H] += 2
	...
	def prettyFormula(self):
			...
			for i in range(0,len(atoms)): 
				if atoms[i] >= 1: formulaStr +=  atomNames[i]
				if atoms[i] >  1: formulaStr  +=  str(atoms[i])
	...
	def printNist(self):
		...
		if (bbType != "bbIsoprenoid"):
			...
			for i in range(0, Nchains):
				...
				d = self.chains[i][1]
				chainstr = ""
				if LIPID_BACKBONES[self.lipidclass][1][i] == "alkyl": #to name plasmalogens ets with p and o convention
					if d > 0:
						chainstr += "p-"
						d -= 1
					else:
				...
				if i == 0 and LIPID_BACKBONES[self.lipidclass][1][0] == "sbase": #if sn1 is a sphingoid base
					h[0] += 1
				if h[i] > 0:
					chainstr += ";O"
				if h[i] >1:
			...
			if LIPID_BACKBONES[self.lipidclass][1][0] == "sbase":
				totalH += 1
			SumName = re.sub("Alkyl_", "", self.lipidclass) + "("  # to removed Alkyl labels from plasmalogens
			...
			name = re.sub("Alkyl_", "", self.lipidclass)	# to removed Alkyl labels from plasmalogens
	...
	#to add together fragments that have same mass (e.g. two acyl groups of same composition)
		...
		if True:
			...
			while i < (len(FRAGMENTS_3)):
				...
				FRAGMENTS_3[i] = FRAGMENTS_3[i] + [float(re.sub("\{|\}", "",  re.search(r"\{(.*?)\}", FRAGMENTS_3[i][2]).group(0)))]
				i = i +1
			...
			for mz in unique_MS2_prec:
				...
				while i < (len(FRAGMENTS_3)-1):
					if FRAGMENTS_3[i][3] != mz or FRAGMENTS_3[i+1][3] != mz:
						i=i+1
						continue
					elif (abs(FRAGMENTS_3[i][0] - FRAGMENTS_3[i + 1][0]) < 0.0001):  # combine if difference in mz is less than 0.1 millidaltons
						FRAGMENTS_3[i][1] = FRAGMENTS_3[i][1] + FRAGMENTS_3[i + 1][1]
						FRAGMENTS_3[i][2] = FRAGMENTS_3[i][2] + "/" + FRAGMENTS_3[i + 1][2]
						del FRAGMENTS_3[i + 1]
					else:
						i = i + 1
		...
		RECORD.append( "PrecursorMz: " +   str((MASS + ADDUCT[self.adduct])/abs(ADDUCT_CHARGE[self.adduct])) + "\n")
