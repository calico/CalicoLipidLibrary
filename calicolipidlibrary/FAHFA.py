...
class FAHFA(GPL):
    ...
    def theoreticalDigest(self):
        ...
        if adduct in NEG_ADDUCTS:
            ...
            FRAGMENTS.append(
                [NL(self.chains[1]) - MW("H2") + H2O - PROTON, 188, "sn2 + water"]
            )
        ...
        if adduct in POS_ADDUCTS:
            if adduct == "[M+Na]+" or adduct == "[M+K]+" or adduct == "[M+NH4]+":
                ...
                FRAGMENTS.append(
                    [NL(self.chains[1]) - MW("H2") + ADDUCT[adduct], 21, "sn2 + adduct"]
                )
                FRAGMENTS.append(
                    [
                        NL(self.chains[1]) - MW("H2") + H2O + ADDUCT[adduct],
                        1,
                        "sn2 + water + adduct",
                    ]
                )
                FRAGMENTS.append(
                    [NL(self.chains[0]) + ADDUCT[adduct], 8, "sn1 + adduct"]
                )
